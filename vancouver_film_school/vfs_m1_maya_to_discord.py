...
# Settings
gt_mtod_settings_mod1 = { 'discord_webhook':hard_coded_webhook,
...
gt_mtod_settings_mod1_default = copy.deepcopy(gt_mtod_settings_mod1)   
...
def get_persistent_settings_maya_to_discord():
    ...
    #Discord Settings # MODIFIED
    if stored_webhook_exists:  
        gt_mtod_settings_mod1['discord_webhook'] = hard_coded_webhook # MODIFIED
        
        if stored_webhook_name_exists and str(cmds.optionVar(q=("gt_maya_to_discord_webhook_name"))) != '':
            gt_mtod_settings_mod1['discord_webhook_name'] = hard_coded_webhook_name # MODIFIED
    else:
        gt_mtod_settings_mod1['is_first_time_running'] = True
   
    if stored_custom_username_exists:  
        gt_mtod_settings_mod1['custom_username'] = str(cmds.optionVar(q=("gt_maya_to_discord_custom_username")))
    else:
        gt_mtod_settings_mod1['custom_username'] = ''
        
    # Image Settings
    if stored_image_format_exists:
        gt_mtod_settings_mod1['image_format'] = str(cmds.optionVar(q=("gt_maya_to_discord_image_format")))
    

    # Playblast Settings
    if stored_image_format_exists:
        gt_mtod_settings_mod1['video_format'] = str(cmds.optionVar(q=("gt_maya_to_discord_video_format")))

    if stored_video_scale_exists:
        gt_mtod_settings_mod1['video_scale_pct'] = int(cmds.optionVar(q=("gt_maya_to_discord_video_scale")))

    if stored_video_compression_exists:
        gt_mtod_settings_mod1['video_compression'] = str(cmds.optionVar(q=("gt_maya_to_discord_video_compression")))
        
    if stored_video_output_type_exists:
        gt_mtod_settings_mod1['video_output_type'] = str(cmds.optionVar(q=("gt_maya_to_discord_video_output_type")))
    
    # Checkboxes
    if stored_feedback_visibility_exists:
        gt_mtod_settings_mod1['feedback_visibility'] = bool(cmds.optionVar(q=("gt_maya_to_discord_feedback_visibility")))
  
    if stored_timestamp_visibility_exists:
        gt_mtod_settings_mod1['timestamp_visibility'] = bool(cmds.optionVar(q=("gt_maya_to_discord_timestamp_visibility")))
...
def set_persistent_settings_maya_to_discord(custom_username, webhook, image_format, video_format, video_scale, video_compression, video_output_type):
    ...
    if webhook != '':  
        ...
        gt_mtod_settings_mod1['discord_webhook'] = str(cmds.optionVar(q=("gt_maya_to_discord_webhook")))
        
        response = discord_get_webhook_name(webhook)
        cmds.optionVar( sv=('gt_maya_to_discord_webhook_name', response) )
        gt_mtod_settings_mod1['discord_webhook_name'] = str(cmds.optionVar(q=("gt_maya_to_discord_webhook_name")))
...
def reset_persistent_settings_maya_to_discord():
    ...
    gt_mtod_settings_mod1['feedback_visibility'] = gt_mtod_settings_mod1_default.get('feedback_visibility')
    gt_mtod_settings_mod1['timestamp_visibility'] = gt_mtod_settings_mod1_default.get('timestamp_visibility')
...
def build_gui_maya_to_discord():
    ...
    if gt_mtod_settings_mod1['is_first_time_running'] == True:
        cmds.text(webhook_name_text, e=True, l=hard_coded_webhook_name)#, bgc= [1,1,0]) # MODIFIED
    else:
        if 'Error' in gt_mtod_settings_mod1.get('discord_webhook_name') or 'Missing Webhook' in gt_mtod_settings_mod1.get('discord_webhook_name'):
            cmds.text(webhook_name_text, e=True, l=gt_mtod_settings_mod1.get('discord_webhook_name'), bgc=[.5,0,0])
        else:
            cmds.text(webhook_name_text, e=True, l=gt_mtod_settings_mod1.get('discord_webhook_name'), nbg=True)
    ...
    def get_username():
        ...
        ''' 
        if gt_mtod_settings_mod1.get('custom_username') == '':
            user_name = socket.gethostname()
        else:
            user_name = gt_mtod_settings_mod1.get('custom_username') + ' (' + socket.gethostname() + ')'
            ...
            if status_value in success_codes: 
                try:  
                    ...
                    if upload_message.strip() != '': 
                        def threaded_upload():
                            try:
                                discord_post_message(get_username(), upload_message, gt_mtod_settings_mod1.get('discord_webhook'))
                                utils.executeDeferred(response_inview_feedback, operation_name, response, display_inview=gt_mtod_settings_mod1.get('feedback_visibility'))
                        ...
                        response_inview_feedback(operation_name, response, display_inview=gt_mtod_settings_mod1.get('feedback_visibility'))
                except:
                    pass
    ...
    def send_dekstop_screenshot():
        ...
        if gt_mtod_settings_mod1.get('is_new_instance'):
            update_discord_webhook_validity(gt_mtod_settings_mod1.get('discord_webhook'))
        
        if gt_mtod_settings_mod1.get('is_webhook_valid'):
            try:
                update_text_status()
                temp_path = generate_temp_file(gt_mtod_settings_mod1.get('image_format'))
                temp_desktop_ss_file = capture_desktop_screenshot(temp_path)
                if gt_mtod_settings_mod1.get('timestamp_visibility'):
                ...
                def threaded_upload():
                    try:
                        ...
                        response = discord_post_attachment(get_username(), upload_message, temp_desktop_ss_file, gt_mtod_settings_mod1.get('discord_webhook'))
        ...
        if gt_mtod_settings_mod1.get('is_webhook_valid'):
            try:  
                ...
                def threaded_upload():
                    try:
                        utils.executeDeferred(disable_buttons)
                        response = discord_post_attachment(get_username(), upload_message, temp_img_file, gt_mtod_settings_mod1.get('discord_webhook'))
                        utils.executeDeferred(enable_buttons)
                        utils.executeDeferred(parse_sending_response, response)
    ...
    def send_animated_playblast():
        ...
        if gt_mtod_settings_mod1.get('is_webhook_valid'):
            try:
                ...
                temp_path = generate_temp_file( gt_mtod_settings_mod1.get('video_format'), file_name=current_scene_name)
                disable_buttons() # This needs to happen before creating the playblast to avoid multiple clicks
                temp_playblast_file = capture_playblast_animation(temp_path, gt_mtod_settings_mod1.get('video_scale_pct'), gt_mtod_settings_mod1.get('video_compression'), gt_mtod_settings_mod1.get('video_output_type') )
                ...
                def threaded_upload():
                    try:
                        response = discord_post_attachment(get_username(), upload_message, temp_playblast_file, gt_mtod_settings_mod1.get('discord_webhook'))
                        utils.executeDeferred(enable_buttons)
                        utils.executeDeferred(parse_sending_response, response)
            ...
            try:  
                ...
                if upload_message.strip() != '':
                    ...
                    def threaded_upload():
                        try:
                            utils.executeDeferred(disable_buttons)
                            response = discord_post_message(get_username(), upload_message, gt_mtod_settings_mod1.get('discord_webhook'))
                            utils.executeDeferred(enable_buttons)
                            utils.executeDeferred(parse_sending_response, response)
                            utils.executeDeferred(response_inview_feedback, 'message', response, display_inview=gt_mtod_settings_mod1.get('feedback_visibility'))
                            utils.executeDeferred(clear_attached_message, response)
                        except:
                            update_text_status(error=True)
                            cmds.warning(webhook_error_message)
        ...
        if gt_mtod_settings_mod1.get('is_webhook_valid'):
            ...
            if len(selection) > 0:
                try:
                    ...
                    def threaded_upload():
                        try:
                            response = discord_post_attachment(get_username(), upload_message, temp_exported_obj, gt_mtod_settings_mod1.get('discord_webhook'))
                            utils.executeDeferred(enable_buttons)
                            utils.executeDeferred(parse_sending_response, response)
        ...
        if gt_mtod_settings_mod1.get('is_webhook_valid'):
            ...
            if len(selection) > 0:
                try:
                    ...
                    def threaded_upload():
                        try:
                            response = discord_post_attachment(get_username(), upload_message, temp_path, gt_mtod_settings_mod1.get('discord_webhook'))
                            utils.executeDeferred(enable_buttons)
                            utils.executeDeferred(parse_sending_response, response)
...
def build_gui_settings_maya_to_discord():
    ...
    # Current Settings =================
    current_image_format = gt_mtod_settings_mod1.get('image_format')
    current_video_format = gt_mtod_settings_mod1.get('video_format')
    current_webhook = ''
    current_custom_username = ''
    if not gt_mtod_settings_mod1.get('is_first_time_running'):
        if gt_mtod_settings_mod1.get('discord_webhook') != '':
            current_webhook = gt_mtod_settings_mod1.get('discord_webhook')
        if gt_mtod_settings_mod1.get('custom_username') != '':
            current_custom_username = gt_mtod_settings_mod1.get('custom_username')
    current_video_scale = gt_mtod_settings_mod1.get('video_scale_pct')
    current_compression = gt_mtod_settings_mod1.get('video_compression')
    current_output_type = gt_mtod_settings_mod1.get('video_output_type')
    ...
    feedback_visibility_chk = cmds.checkBox(label='', value=gt_mtod_settings_mod1.get('feedback_visibility'), cc=lambda args: update_checkbox_settings_data())
    cmds.text(l='Display Viewport Feedback', align="left", font= 'smallPlainLabelFont')
    timestamp_visibility_chk = cmds.checkBox(label='', value=gt_mtod_settings_mod1.get('timestamp_visibility'), cc=lambda args: update_checkbox_settings_data())
    ...
    def reset_settings():
        ...
        cmds.textField(new_username_input, e=True, text=gt_mtod_settings_mod1_default.get('custom_username'))
        cmds.textField(new_image_format_input, e=True, text=gt_mtod_settings_mod1_default.get('image_format'))
        cmds.textField(new_video_format_input, e=True, text=gt_mtod_settings_mod1_default.get('video_format') )
        
        for idx,obj in enumerate(cmds.optionMenu(output_type_input,q=True, itemListLong=True)):
            if cmds.menuItem( obj , q=True, label=True ) == gt_mtod_settings_mod1_default.get('video_output_type'):
                cmds.optionMenu(output_type_input, e=True, select=idx+1)
        ...
        found_default = False
        for idx,obj in enumerate(cmds.optionMenu(compression_input, q=True, itemListLong=True)):
            if cmds.menuItem( obj , q=True, label=True ) == gt_mtod_settings_mod1_default.get('video_compression'):
        ...
        cmds.intSliderGrp(video_scale_input, e=True, value=gt_mtod_settings_mod1_default.get('video_scale_pct'))
        
        # Check box Management
        cmds.checkBox(feedback_visibility_chk, e=True, value=gt_mtod_settings_mod1_default.get('feedback_visibility'))
        cmds.checkBox(timestamp_visibility_chk, e=True, value=gt_mtod_settings_mod1_default.get('timestamp_visibility'))
    ...
    def apply_settings():
        ...
        gt_mtod_settings_mod1['is_first_time_running'] = False
        gt_mtod_settings_mod1['is_new_instance'] = True
...
def capture_playblast_animation(video_file, scale_pct, compression, video_format): 
    ...
    if gt_mtod_settings_mod1.get('feedback_visibility'):
...
def update_discord_webhook_validity(webhook_url):
    ...
    try: 
        ...
        if response.status in success_codes: 
            ...
            gt_mtod_settings_mod1['is_new_instance'] = False
            gt_mtod_settings_mod1['is_webhook_valid'] = True 
        else:
            gt_mtod_settings_mod1['is_new_instance'] = False
            gt_mtod_settings_mod1['is_webhook_valid'] = False 
    except:
        gt_mtod_settings_mod1['is_new_instance'] = False
        gt_mtod_settings_mod1['is_webhook_valid'] = False 
